...
import { Download, Check, RefreshCw, ArrowLeft, FileText, Sparkles, Trophy } from 'lucide-react';
...
const TORAH_SUPPORT_BG_URL = "https://res.cloudinary.com/dpuzslr2j/image/upload/v1770749574/Gemini_Generated_Image_hz12ghz12ghz12gh_zu008h.png";
...
const TEMPLATES = [
  ...
  },
  {
    ...
    getFields: (h) => [],
    render: (ctx, width, height, data, logo, decoration, extra) => {
      if (extra?.torahBg && extra.torahBg.complete && extra.torahBg.naturalWidth !== 0) {
        const bg = extra.torahBg;
        ...
        ctx.font = `bold 40px ${FONT_BODY}`;
        ctx.textAlign = 'center';
        ctx.fillText("Loading Background...", width / 2, height / 2);
      }

      const activeLogo = extra?.coloredLogo;
      if (activeLogo) {
        const logoW = 350;
        const logoH = (activeLogo.height / activeLogo.width) * logoW;
        ctx.drawImage(activeLogo, (width - logoW) / 2, height * 0.05, logoW, logoH);
      }
    }
  }
];
...
function App() {
  ...
  const [torahBgImage, setTorahBgImage] = useState(null);
  ...
  useEffect(() => {
    ...
    if (TORAH_SUPPORT_BG_URL) {
      const torahBg = new Image();
      torahBg.src = TORAH_SUPPORT_BG_URL;
      torahBg.crossOrigin = "anonymous";
      torahBg.onload = () => setTorahBgImage(torahBg);
    }
  }, []);
  ...
  const draw = useCallback(() => {
    ...
    if (template && template.render) {
      template.render(
        ...
        logoImage,
        decorationImage,
        { coloredLogo: coloredLogoImage, torahBg: torahBgImage }
      );
    }
  }, [selectedTemplate, formData, logoImage, decorationImage, coloredLogoImage, torahBgImage]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 font-sans text-slate-100 pb-20 selection:bg-amber-500/30 selection:text-amber-100">
      ...
      <header className="fixed top-0 w-full z-50 px-6 py-4 flex items-center justify-between bg-slate-950/80 backdrop-blur-md border-b border-slate-800 shadow-lg">
        <div
          ...
          onClick={handleBack}
        >
          <div className="relative w-14 h-14 rounded-2xl overflow-hidden border border-slate-700 group-hover:border-amber-500/50 shadow-inner bg-slate-900 transition-all duration-300 group-hover:scale-105">
            {(appMode === 'editor' || (appMode === 'home' && category)) ? (
              <div className="absolute inset-0 flex items-center justify-center bg-slate-800 text-slate-300 group-hover:text-amber-400">
                <ArrowLeft size={24} />
              </div>
            ) : (
              <img src={APP_LOGO_URL} alt="Logo" className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity" />
            )}
          </div>
          <div>
            <h1 className="font-bold text-lg tracking-wide text-[#1e3a8a] group-hover:text-amber-400 transition-colors duration-300 font-serif">Yeshivas Zerach</h1>
      ...
      <main className="max-w-md mx-auto pt-28 px-4 flex flex-col gap-8 min-h-screen">
              ...
              >
                ...
                <div className="flex items-center gap-5 relative z-10">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-slate-400 mt-1 font-medium tracking-wide uppercase">Track Campaign Progress</p>
                  </div>
                </div>
              </button>
              {/* ------------------------ */}
        ...
        {appMode === 'home' && category === 'milestones' && (
          /* --- MILESTONES VIEW --- */
          <div className="flex flex-col gap-6 animate-scale-in">
            ...
            <div className="p-6 bg-slate-900/50 rounded-xl border border-slate-700/50 text-center space-y-4">
              <Trophy size={48} className="mx-auto text-purple-500 animate-bounce" />
              <h3 className="text-xl font-bold text-white">Campaign Goals</h3>
              <p className="text-slate-400 text-sm">Milestone tracking features coming soon!</p>
              <div className="w-full bg-slate-800 rounded-full h-2.5 dark:bg-slate-700 overflow-hidden">
                <div className="bg-purple-600 h-2.5 rounded-full" style={{ width: '45%' }}></div>
              </div>
              <p className="text-xs text-slate-500 font-mono">45% CONNECTED</p>
            </div>
          </div>
        )}

        {appMode === 'home' && category && category !== 'milestones' && (
          /* --- GRID VIEW (Filtered) --- */
          <div className="flex flex-col gap-6 animate-scale-in">
            <div className="flex items-center justify-between pb-2 border-b border-slate-800/50">
              <h2 className="text-2xl font-bold text-slate-100 capitalize font-serif">{category}</h2>
                ...
                >
                  <div className="absolute inset-0 opacity-80 group-hover:opacity-100 transition-opacity duration-500">
                    <TemplateThumbnail
                      ...
                      extra={{ coloredLogo: coloredLogoImage, torahBg: torahBgImage }}
                    />
                  </div>
